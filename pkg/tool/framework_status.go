...
func generateStatusReport(fwDir string, components []componentStatus) string {
	...
	sb.WriteString(fmt.Sprintf("> **Auto-generated:** %s\n", time.Now().UTC().Format("2006-01-02 15:04:05 UTC")))
	sb.WriteString(fmt.Sprintf("> **Framework root:** `%s`\n", fwDir))
	sb.WriteString(fmt.Sprintf("> **Source:** `framework_status` tool\n\n"))
	...
	sb.WriteString(fmt.Sprintf("| ✅ Functional | %d | Real implementation, working |\n", functional))
	sb.WriteString(fmt.Sprintf("| ⚠️ Partial | %d | Has code but contains TODOs or incomplete |\n", partial))
	sb.WriteString(fmt.Sprintf("| ❌ Not Implemented | %d | Stub / placeholder / print-only |\n", notImpl))
	sb.WriteString(fmt.Sprintf("| 📝 Planned | %d | Minimal file, early stage |\n", planned))
	sb.WriteString(fmt.Sprintf("| **Total** | **%d** | |\n", len(components)))
	sb.WriteString(fmt.Sprintf("| ✅ Has tests | %d / %d | |\n\n", totalTested, len(components)))
	...
	sb.WriteString(fmt.Sprintf("**Health Score:** %d%% functional (%d/%d components)\n\n", healthScore, functional, len(components)))
...
// scanFileForIssue scans a single file for stub patterns (used by the inline scanner).
func scanFileForIssue(path string) (string, bool) {
	f, err := os.Open(path)
	if err != nil {
		return "", false
	}
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, "Note: Real implementation would") ||
			strings.Contains(line, "Note: Real attack would") ||
			strings.Contains(line, "Note: Exodus stores") ||
			strings.Contains(line, "Injection prepared") ||
			strings.Contains(line, "Remote thread creation ready") ||
			strings.Contains(line, "not implemented") && strings.Contains(line, "fmt.Errorf") {
			return fmt.Sprintf("Line %d: %s", lineNum, strings.TrimSpace(line)), true
		}
	}
